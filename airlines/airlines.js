...
function infoScaleFlights() {
    for (let i = 0; i < flights.length; i++) {
        if (flights[i].scale === false) {
            console.log(`The flight #${flights[i].id} from ${flights[i].from} to ${flights[i].to} has a cost of ${flights[i].cost} € and there's no scale.`);
        } else {
            console.log(`The flight #${flights[i].id} from ${flights[i].from} to ${flights[i].to} has a cost of ${flights[i].cost} € and there's a scale.`);
        }
    }
}
