...
function infoScaleFlights() {
    for (let i = 0; i < flights.length; i++) {
        if (flights[i].scale === false) {
            console.log(`The flight #${flights[i].id} from ${flights[i].from} to ${flights[i].to} has a cost of ${flights[i].cost} € and there's no stopover.`);
        } else {
            console.log(`The flight #${flights[i].id} from ${flights[i].from} to ${flights[i].to} has a cost of ${flights[i].cost} € and there's a stopover.`);
        }
    }
}
infoScaleFlights();
...
function askUser() {
    ...
    if (userAdmin == "admin") {
        let menuAdmin = Number(prompt("What would you like to do?\n\n1. Create a flight\n2. Delete a flight\n3. See flights\n4. Back to main menu\n5. Exit"));
        if (menuAdmin == 1) {
            crearVuelos();
            askUser();
        } else if (menuAdmin == 2) {
        ...
        let menuUser = Number(prompt("What would you like to do?\n\n1. Search flight by price\n2. See flights\n3. Back to main menu\n4. Exit"));
        if (menuUser == 1) {
            filterFlights();
            askUser();
        } else if (menuUser == 2) {
...
function filterFlights() {
    costUser = prompt("Enter an amount and we'll show you flights with the same price or lower.");
...
function deleteFlights(){
    idFlightToDelete = Number(prompt("Enter the number of the flight you want to delete"));
