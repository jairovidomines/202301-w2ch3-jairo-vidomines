...
const confirmCard = () => {
  const confirm = prompt(
    "¿Do you want to change the card numbers? yes/no"
  ).toLowerCase();
  if (confirm == "yes") {
...
const ballRolling = () => {
  const numberBall = numbersArray2[Math.floor(Math.random() * 99)];
  if (isNaN(numberBall)) {
    ballRolling();
  } else {
    console.log("‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾");
